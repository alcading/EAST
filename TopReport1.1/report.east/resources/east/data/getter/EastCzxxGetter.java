...
public class EastCzxxGetter extends BaseGetter {
	/*
	 * 获取列表
	 * 
	 * @author 
	...
	public Result call() throws AppException {
		try {
			...
			this.setValue2DataBus(ReportConstant.QUERY_LOG_BUSI_NAME, "存折信息");
	...
	private PageQueryResult getData() throws CommonException {
	    ...
	    String czh = (String)para.get("czh");
		...
		hql.append("from EastCzxx A where rownum<="+DBUtil.ROWNUM);
		
		if(StringUtils.isNotBlank(czh)){
			hql.append(" and A.id.czh = '"+czh.trim()+"' ");
		}
		if(StringUtils.isNotBlank(hqckzh)){
			hql.append(" and A.hqckzh = '"+hqckzh.trim()+"' ");
		}
		...
		PageQueryResult pqr = EastCzxxService.getInstance().pageQueryByHql(pageIndex, pageSize, hql.toString());
		return pqr;
	}
}
