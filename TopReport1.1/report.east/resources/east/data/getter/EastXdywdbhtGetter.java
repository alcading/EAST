...
public class EastXdywdbhtGetter extends BaseGetter {
	/*
	 * 获取列表
	 * 
	 * @author 
	...
	public Result call() throws AppException {
		try {
			...
			this.setValue2DataBus(ReportConstant.QUERY_LOG_BUSI_NAME, "担保关系");
	...
	private PageQueryResult getData() throws CommonException {
	    ...
	    String bzrmc = (String)para.get("bzrmc");
		...
		hql.append("from EastXdywdbht A where rownum<="+DBUtil.ROWNUM);
		
		if(StringUtils.isNotBlank(dbhth)){
			hql.append(" and A.id.dbhth = '"+dbhth.trim()+"' ");
		}
		if(StringUtils.isNotBlank(bzrmc)){
			hql.append(" and A.bzrmc like '%"+bzrmc.trim()+"%' ");
		}
		...
		PageQueryResult pqr = EastXdywdbhtService.getInstance().pageQueryByHql(pageIndex, pageSize, hql.toString());
		return pqr;
	}
}
