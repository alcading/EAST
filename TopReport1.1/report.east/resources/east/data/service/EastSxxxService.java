...
public class EastSxxxService {
	// 根据查询hql语句进行分页查询
		...
		public static EastSxxxService getInstance() {
			return (EastSxxxService) ApplicationContextUtils.getBean("EastSxxxService");
		}
		...
		public Iterator selectByid(String id) {
			...
			try {
				return rootDAO.queryByQL("from EastSxxx A where A.id='" + id + "'");
		...
		// 通过id来获取实体类
		public EastSxxx selectById(String id) {
			ROOTDAO rootdao = ROOTDAOUtils.getROOTDAO();
			EastSxxx eastXdhtb = null;
			try {

				eastXdhtb = (EastSxxx) rootdao.query(EastSxxx.class, id);
		 ...
		 */
		public void removeEntity(EastSxxx eastXmdkxxb) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
		...
		public void addEntity(EastSxxx eastXmdkxxb) throws CommonException {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
		...
		public void modEntity(EastSxxx eastXmdkxxb) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
		...
		public boolean isExists(EastSxxxPK id) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
				EastSxxx bd = (EastSxxx) rootDAO.query(EastSxxx.class, id);
				if (bd == null) {
					return false;
				}
			} catch (CommonException e) {
