...
public class EastJjkxxService {
	// 根据查询hql语句进行分页查询
		...
		public static EastJjkxxService getInstance() {
			return (EastJjkxxService) ApplicationContextUtils.getBean("EastJjkxxService");
		}
		...
		public Iterator selectByid(String id) {
			...
			try {
				return rootDAO.queryByQL("from EastJjkxx A where A.id='" + id + "'");
		...
		// 通过id来获取实体类
		public EastJjkxx selectById(String id) {
			ROOTDAO rootdao = ROOTDAOUtils.getROOTDAO();
			EastJjkxx eastXdhtb = null;
			try {

				eastXdhtb = (EastJjkxx) rootdao.query(EastJjkxx.class, id);
		 ...
		 */
		public void removeEntity(EastJjkxx eastXmdkxxb) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
		...
		public void addEntity(EastJjkxx eastJjkxx) throws CommonException {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			if (isExists(eastJjkxx.getId())) {
				ExceptionUtil.throwCommonException(" 机构重复");
			}
			try {
				rootDAO.save(eastJjkxx);
		...
		public void modEntity(EastJjkxx eastXmdkxxb) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
		...
		public boolean isExists(EastJjkxxPK id) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
				EastJjkxx bd = (EastJjkxx) rootDAO.query(EastJjkxx.class, id);
				if (bd == null) {
					return false;
				}
			} catch (CommonException e) {
