...
public class EastDbgxService {
	// 根据查询hql语句进行分页查询
		...
		public static EastDbgxService getInstance() {
			return (EastDbgxService) ApplicationContextUtils.getBean("EastDbgxService");
		}
		...
		public Iterator selectByid(String id) {
			...
			try {
				return rootDAO.queryByQL("from EastDbgx A where A.id='" + id + "'");
		...
		// 通过id来获取实体类
		public EastDbgx selectById(String id) {
			ROOTDAO rootdao = ROOTDAOUtils.getROOTDAO();
			EastDbgx eastXdhtb = null;
			try {

				eastXdhtb = (EastDbgx) rootdao.query(EastDbgx.class, id);
		 ...
		 */
		public void removeEntity(EastDbgx eastXmdkxxb) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
		...
		public void addEntity(EastDbgx eastXmdkxxb) throws CommonException {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
		...
		public void modEntity(EastDbgx eastXmdkxxb) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
		...
		public boolean isExists(EastDbgxId id) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
				EastDbgx bd = (EastDbgx) rootDAO.query(EastDbgx.class, id);
				if (bd == null) {
					return false;
				}
			} catch (CommonException e) {
