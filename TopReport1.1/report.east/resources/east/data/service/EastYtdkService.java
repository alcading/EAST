...
import resources.east.data.pub.EastYtdkPK;
...
public class EastYtdkService {
	// 根据查询hql语句进行分页查询
		...
		public static EastYtdkService getInstance() {
			return (EastYtdkService) ApplicationContextUtils.getBean("EastYtdkService");
		}
		...
		public Iterator selectByid(String id) {
			...
			try {
				return rootDAO.queryByQL("from EastYtdk A where A.id='" + id + "'");
		...
		// 通过id来获取实体类
		public EastYtdk selectById(String id) {
			ROOTDAO rootdao = ROOTDAOUtils.getROOTDAO();
			EastYtdk eastXdhtb = null;
			try {

				eastXdhtb = (EastYtdk) rootdao.query(EastYtdk.class, id);
		 ...
		 */
		public void removeEntity(EastYtdk eastXmdkxxb) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
		...
		public void addEntity(EastYtdk eastXmdkxxb) throws CommonException {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
		...
		public void modEntity(EastYtdk eastXmdkxxb) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
		...
		public boolean isExists(EastYtdkPK id) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
				EastYtdk bd = (EastYtdk) rootDAO.query(EastYtdk.class, id);
				if (bd == null) {
					return false;
				}
			} catch (CommonException e) {
