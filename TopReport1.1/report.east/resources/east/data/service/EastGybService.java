...
public class EastGybService {
	...
	public static EastGybService getInstance() {
		return (EastGybService) ApplicationContextUtils.getBean("EastGybService");
	}
	...
	public Iterator selectByid(String id) {
		...
		try {
			return rootDAO.queryByQL("from EastGyb eastGyb where eastGyb.id='" + id + "'");
	...
	// 通过id来获取实体类
	public EastGyb selectById(String id) {
		ROOTDAO rootdao = ROOTDAOUtils.getROOTDAO();
		EastGyb eastGyb = null;
		try {

			eastGyb = (EastGyb) rootdao.query(EastGyb.class, id);
	 ...
	 */
	public void removeEntity(EastGyb eastGyb) {
		ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
		try {
			rootDAO.delete(eastGyb);
	...
	public void addEntity(EastGyb eastGyb) throws CommonException {
		ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
		if(isExists(eastGyb.getId())) {
			ExceptionUtil.throwCommonException("柜员重复");
		}
		try {
			rootDAO.save(eastGyb);
