...
public class EastCzxxService {
	// 根据查询hql语句进行分页查询
		...
		public static EastCzxxService getInstance() {
			return (EastCzxxService) ApplicationContextUtils.getBean("EastCzxxService");
		}
		...
		public Iterator selectByid(String id) {
			...
			try {
				return rootDAO.queryByQL("from EastCzxx A where A.id='" + id + "'");
		...
		// 通过id来获取实体类
		public EastCzxx selectById(String id) {
			ROOTDAO rootdao = ROOTDAOUtils.getROOTDAO();
			EastCzxx eastXdhtb = null;
			try {

				eastXdhtb = (EastCzxx) rootdao.query(EastCzxx.class, id);
		 ...
		 */
		public void removeEntity(EastCzxx eastXmdkxxb) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
		...
		public void addEntity(EastCzxx eastCzxx) throws CommonException {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			if (isExists(eastCzxx.getId())) {
				ExceptionUtil.throwCommonException(" 机构重复");
			}
			try {
				rootDAO.save(eastCzxx);
		...
		public void modEntity(EastCzxx eastXmdkxxb) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
		...
		public boolean isExists(EastCzxxTmpId id) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
				EastCzxx bd = (EastCzxx) rootDAO.query(EastCzxx.class, id);
				if (bd == null) {
					return false;
				}
			} catch (CommonException e) {
