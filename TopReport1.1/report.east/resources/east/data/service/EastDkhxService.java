...
public class EastDkhxService {
	// 根据查询hql语句进行分页查询
		...
		public static EastDkhxService getInstance() {
			return (EastDkhxService) ApplicationContextUtils.getBean("EastDkhxService");
		}
		...
		public Iterator selectByid(String id) {
			...
			try {
				return rootDAO.queryByQL("from EastDkhx A where A.id='" + id + "'");
		...
		// 通过id来获取实体类
		public EastDkhx selectById(String id) {
			ROOTDAO rootdao = ROOTDAOUtils.getROOTDAO();
			EastDkhx eastXdhtb = null;
			try {

				eastXdhtb = (EastDkhx) rootdao.query(EastDkhx.class, id);
		 ...
		 */
		public void removeEntity(EastDkhx eastXmdkxxb) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
		...
		public void addEntity(EastDkhx eastXmdkxxb) throws CommonException {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
		...
		public void modEntity(EastDkhx eastXmdkxxb) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
		...
		public boolean isExists(EastDkhxPK id) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
				EastDkhx bd = (EastDkhx) rootDAO.query(EastDkhx.class, id);
				if (bd == null) {
					return false;
				}
			} catch (CommonException e) {
