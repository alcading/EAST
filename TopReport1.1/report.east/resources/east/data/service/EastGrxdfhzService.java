 ...
 *
 */
public class EastGrxdfhzService {
	// 根据查询hql语句进行分页查询
	...
	public static EastGrxdfhzService getInstance() {
	...
	public Iterator selectByid(String id) {
		...
		try {
			return rootDAO.queryByQL("from EastGrxdfhz eastGrxdfhz where eastGrxdfhz.id='" + id + "'");
	...
	// 通过id来获取实体类
	public EastGrxdfhz selectById(String id) {
		ROOTDAO rootdao = ROOTDAOUtils.getROOTDAO();
		EastGrxdfhz eastGrxdfhz = null;
		try {

			eastGrxdfhz = (EastGrxdfhz) rootdao.query(EastGrxdfhz.class, id);
	 ...
	 */
	public void removeEntity(EastGrxdfhz eastGrxdfhz) {
		ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
		try {
			rootDAO.delete(eastGrxdfhz);
	...
	public void addEntity(EastGrxdfhz eastGrxdfhz) throws CommonException {
		ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
		if(isExists(eastGrxdfhz.getId())) {
			ExceptionUtil.throwCommonException(" 机构重复");
		}
		try {
			rootDAO.save(eastGrxdfhz);
	...
	public void modEntity(EastGrxdfhz eastGrxdfhz) {
		ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
		try {
			rootDAO.update(eastGrxdfhz);
	...
	public boolean isExists(EastGrxdfhzId id) {
		ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
		try {
			EastGrxdfhz bd = (EastGrxdfhz) rootDAO.query(EastGrxdfhz.class, id);
			if(bd == null) {
				return false;
			}
		} catch (CommonException e) {
