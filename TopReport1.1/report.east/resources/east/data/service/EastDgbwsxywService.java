 ...
 *
 */
public class EastDgbwsxywService {
	// 根据查询hql语句进行分页查询
		...
		public static EastDgbwsxywService getInstance() {
			return (EastDgbwsxywService) ApplicationContextUtils.getBean("EastDgbwsxywService");
		}
		...
		public Iterator selectByid(String id) {
			...
			try {
				return rootDAO.queryByQL("from EastDgbwsxyw A where A.id='" + id + "'");
		...
		// 通过id来获取实体类
		public EastDgbwsxyw selectById(String id) {
			ROOTDAO rootdao = ROOTDAOUtils.getROOTDAO();
			EastDgbwsxyw eastXdhtb = null;
			try {

				eastXdhtb = (EastDgbwsxyw) rootdao.query(EastDgbwsxyw.class, id);
		 ...
		 */
		public void removeEntity(EastDgbwsxyw eastXmdkxxb) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
		...
		public void addEntity(EastDgbwsxyw eastXmdkxxb) throws CommonException {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
		...
		public void modEntity(EastDgbwsxyw eastXmdkxxb) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
		...
		public boolean isExists(EastDgbwsxywPK id) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
				EastDgbwsxyw bd = (EastDgbwsxyw) rootDAO.query(EastDgbwsxyw.class, id);
				if (bd == null) {
					return false;
				}
			} catch (CommonException e) {
