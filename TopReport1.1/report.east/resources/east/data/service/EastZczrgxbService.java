 ...
 *
 */
public class EastZczrgxbService {
	// 根据查询hql语句进行分页查询
		...
		public static EastZczrgxbService getInstance() {
			return (EastZczrgxbService) ApplicationContextUtils.getBean("EastZczrgxbService");
		}
		...
		public Iterator selectByid(String id) {
			...
			try {
				return rootDAO.queryByQL("from EastZczrgxb A where A.id='" + id + "'");
		...
		// 通过id来获取实体类
		public EastZczrgxb selectById(String id) {
			ROOTDAO rootdao = ROOTDAOUtils.getROOTDAO();
			EastZczrgxb eastXdhtb = null;
			try {

				eastXdhtb = (EastZczrgxb) rootdao.query(EastZczrgxb.class, id);
		 ...
		 */
		public void removeEntity(EastZczrgxb eastXmdkxxb) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
		...
		public void addEntity(EastZczrgxb eastXmdkxxb) throws CommonException {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
		...
		public void modEntity(EastZczrgxb eastXmdkxxb) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
		...
		public boolean isExists(EastZczrgxbPK id) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
				EastZczrgxb bd = (EastZczrgxb) rootDAO.query(EastZczrgxb.class, id);
				if (bd == null) {
					return false;
				}
			} catch (CommonException e) {
