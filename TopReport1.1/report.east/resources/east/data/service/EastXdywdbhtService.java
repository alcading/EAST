 ...
 *
 */
public class EastXdywdbhtService {
	// 根据查询hql语句进行分页查询
		...
		public static EastXdywdbhtService getInstance() {
			return (EastXdywdbhtService) ApplicationContextUtils.getBean("EastXdywdbhtService");
		}
		...
		public Iterator selectByid(String id) {
			...
			try {
				return rootDAO.queryByQL("from EastXdywdbht A where A.id='" + id + "'");
		...
		// 通过id来获取实体类
		public EastXdywdbht selectById(String id) {
			ROOTDAO rootdao = ROOTDAOUtils.getROOTDAO();
			EastXdywdbht eastXdhtb = null;
			try {

				eastXdhtb = (EastXdywdbht) rootdao.query(EastXdywdbht.class, id);
		 ...
		 */
		public void removeEntity(EastXdywdbht eastXmdkxxb) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
		...
		public void addEntity(EastXdywdbht eastXmdkxxb) throws CommonException {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
		...
		public void modEntity(EastXdywdbht eastXmdkxxb) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
		...
		public boolean isExists(EastXdywdbhtPK id) {
			ROOTDAO rootDAO = ROOTDAOUtils.getROOTDAO();
			try {
				EastXdywdbht bd = (EastXdywdbht) rootDAO.query(EastXdywdbht.class, id);
				if (bd == null) {
					return false;
				}
			} catch (CommonException e) {
