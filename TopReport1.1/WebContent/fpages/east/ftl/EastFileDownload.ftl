...
<@CommonQueryMacro.page title="下载报文">
	<@CommonQueryMacro.CommonQuery id="EastFileDownload" init="false">
		<@CommonQueryMacro.ToolBar id="EastFileDownload">
			<table width="100%">
				<tr>
...
<script>
	window.onload=function(){
	    ...
	    EastFileDownload_interface_dataset.setValue("sjrq", currentDate);
	
	}
	// 下载
	function btLoad_onClickCheck(button){
        var sjrq = EastFileDownload_interface_dataset.getValue("sjrq");
        if(null == sjrq || '' == sjrq){
        	easyMsg.info("请选择数据日期！");
        	return false;
        }
		document.getElementById("filedownloadfrm").src ="${contextPath}/EastFileLoad?sjrq="+sjrq;
		return false;
  	}
</script>
