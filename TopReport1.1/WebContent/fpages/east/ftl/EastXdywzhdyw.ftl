...
<@CommonQueryMacro.CommonQuery id="EastXdywzhdyw" init="true" submitMode="current">
<table width="1300px">
	<tr>
...
<script language="JavaScript">
	...
	window.onload=function(){
	    ...
	    EastXdywzhdyw_interface_dataset.setValue("cjrq", enddate);
	}

	//定位一行记录
	function locate(id) {
		var record = EastXdywzhdyw_dataset.find(["id"],[id]);
		if(record) {
			EastXdywzhdyw_dataset.setRecord(record);
		}
	}
	//系统刷新单元格
	...
	function signWindow_floatWindow_beforeClose(subwindow) {
		EastXdywzhdyw_dataset.cancelRecord();
		return true;
	}
	...
	function openModifyWindow(id,flag) {
		locate(id);
		if(flag==1){
		EastXdywzhdyw_dataset.setFieldReadOnly("cjrq",true);
	...
	function btModOrAdd_onClickCheck(button) {
		var id = EastXdywzhdyw_dataset.getValue("id");
		if(id == null || "" == id ) {
			alert("字段[内部机构号]不能为空");
			return false;
		}
	...
	function flushCurrentPage() {
		EastXdywzhdyw_dataset.flushData(EastXdywzhdyw_dataset.pageIndex);
	}
</script>
